...
| 9 | Credencial indo pro Drive | `sensitive = {"password","code-server-pw"}` excluídos do sync | `src/daemon.py:585` |
...
| 13 | Senha fraca / previsível | `secrets.choice` 16 chars + idempotente por sessão (arquivo) | célula de config / START |
...
- **Segredo no output do notebook.** A célula de START imprime a senha uma vez para o dono (necessário para logar). Não compartilhe o notebook com output. Senha completa também fica em `~/.colab-roots/state/password` (0600).
